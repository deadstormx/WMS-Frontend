...
const Routes = () => {
  ...
  useEffect(() => {
    const fetchRoutes = async () => {
      try {
        ...
        const response = await fetch('https://wms-backend-xdgv.onrender.com/api/route/get-routes', {
          headers: {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://wms-backend-xdgv.onrender.com/api/route/update-route/${editingRoute.routeId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://wms-backend-xdgv.onrender.com/api/route/add-route', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleStartRoute = async (route) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://wms-backend-xdgv.onrender.com/api/route/start-route/${route.routeId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (result.success) {
        ...
        setTimeout(async () => {
          try {
            const completeResponse = await fetch(`https://wms-backend-xdgv.onrender.com/api/route/update-route/${route.routeId}`, {
              method: 'PUT',
              headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
  ...
  const handleDeleteRoute = async (routeId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://wms-backend-xdgv.onrender.com/api/route/delete-route/${routeId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'

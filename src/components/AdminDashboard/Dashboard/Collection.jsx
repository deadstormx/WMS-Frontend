...
const Collection = () => {
  ...
  const fetchCollections = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('https://wms-backend-xdgv.onrender.com/api/collections', {
        method: "GET",
        headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://wms-backend-xdgv.onrender.com/api/collections/${editingCollection._id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
